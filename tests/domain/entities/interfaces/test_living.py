...
def test__init_with_exception(health_points):
    with pytest.raises(ValueError):
        Living(health_points=health_points)


@pytest.mark.parametrize("health_points,expected", [(1, True), (0, False), (-1, False)])
def test__is_available(health_points, expected):
...
def test__take_damage(start_health_points, damage, result_health_points):
...
def test__take_damage_with_exception(damage):
    with pytest.raises(ValueError):
